...
<ModalCadastrarBaseCadastroTexto
    v-if="modalAbertoTipo"
    texto="Cadastre um novo parentesco"
    @enviar-modal="enviarTipo"
    @fechar-modal="dependenteStore.setModalTipoDependente"
/>
...
const emit = defineEmits(['fechar-modal', 'enviado'])

const pessoaStore = usePessoaStore()
const dependenteStore = useDependenteStore()
const alertStore = useAlertStore()

const modalAbertoTipo = computed(() => dependenteStore.getModalTipo)
...
const enviarTipo = async (dado: string) => {

    const body = {
        descricao: dado
    }

    dependenteStore.fetchCadastrarTipoDependentes(body).then(() => {
        alertStore.mostrarAlerta('success', 'Cadastrado com sucesso!');
        dependenteStore.setModalTipoDependente()
    }).catch(() => {
